...
// Build path -> entry lookup for complete entries only.
const completeEntries = entries.filter(e => e.status === 'complete');
const byPath = new Map(completeEntries.map(e => [e.path, e]));

// For each entry, determine its CN identifier(s) that could be referenced as a chip.
// Characters: e.char. Vocab/chengyu/grammar/topics: derive from title (CN portion before space/punct).
function chipKeysFor(entry) {
  const keys = new Set();
  if (entry.char) keys.add(entry.char);
  if (entry.title) {
    // Title format: "字 · english gloss" or "汉字 hànzì — english". Take leading CN run.
  ...
  // Also derive from filename: pages/category/<slug>_<CN>.html
...
// Count inbound mentions = number of *other* pages that have this entry's CN key
// in their adj-index appearsOn list.
...
// Suggest referrers: for each orphan, find up to 4 other complete pages in the
// same category that share at least one tag, ranked by tag overlap. These are
// candidate pages to add an inbound chip on.
function suggestReferrers(orphan) {
  const orphanTags = new Set(orphan.entry.tags || []);
  if (orphanTags.size === 0) return [];
  const candidates = completeEntries
...
lines.push('');
lines.push('Diagnosis from 2026-05-06: pages with 0 inbound chip mentions are 6/6 not indexed by Google;');
lines.push('pages with ≥1 inbound mention are 6/6 indexed. Adding chip references on existing high-traffic');
lines.push('pages that point *to* an orphan is the cheapest known way to push it into Google\'s index.');
lines.push('');
